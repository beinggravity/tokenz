...
https://github.com/user-attachments/assets/4578280d-5972-4bb7-8e48-f9c2e263c08d
...
[![Version](https://img.shields.io/badge/Version-1.0.0-f7a94a?style=flat-square)](./package.json)
...
> **🎉 Major release — everything below is new since v0.4.0**

### 🗂 Folder Drop Support
You can now drag and drop entire **folders** — including nested sub-folders — directly onto ChatGPT or Claude. Tokenz recursively walks the folder tree, picks up every supported code or text file, and combines them into a single structured message.

### 🧑‍💻 Expanded Code File Support
Tokenz now recognises **40+ coding file extensions** out of the box — including `.js`, `.ts`, `.py`, `.rs`, `.go`, `.java`, `.cpp`, `.cs`, `.swift`, `.kt`, `.rb`, `.php`, `.vue`, `.jsx`, `.tsx`, `.json`, `.yaml`, `.toml`, `.env`, `.sh`, and many more.

### ⚠️ Large Context Warning
When a file or folder exceeds your **Context Limit**, Tokenz pauses and shows you a warning dialog before sending. This prevents accidentally flooding the AI with data that exceeds its context window and degrades performance. You get two choices:
- **Continue anyway** — proceed with auto-splitting and multi-turn delivery.
- **Select fewer files** — cancel and try again with a smaller scope.

### ⚙️ Advanced Settings
A new **Advanced Settings** panel is available directly in the extension popup (click the Tokenz icon → expand *Advanced Settings*). You can now customise:
- **Context Limit** *(default: 65,000 chars)* — the threshold at which the large context warning triggers and auto-splitting begins.
- **Chunk Size** *(default: 10,000 chars)* — the internal paste buffer size per step. All settings save automatically.
