...
export function askConfirm(title, detail) {
  return new Promise((resolve) => {
    ...
    hud.innerHTML = `
      <div class="tokenz-hud-card tokenz-glass" style="pointer-events: auto;">
        <div class="tokenz-hud-title" style="color: #ffb86c; font-size: 18px; margin-bottom: 8px;">${escapeHtml(title)}</div>
        <div class="tokenz-hud-detail" style="margin-bottom: 24px; font-size: 13px;">${escapeHtml(detail)}</div>
        <div style="display: flex; gap: 12px; justify-content: center;">
          <button id="tz-btn-cancel" style="padding: 10px 18px; border-radius: 8px; background: rgba(255,255,255,0.1); color: white; border: 1px solid rgba(255,255,255,0.2); cursor: pointer; font-weight: 600; font-size: 13px; transition: background 0.2s;">Select fewer files</button>
          <button id="tz-btn-ok" style="padding: 10px 18px; border-radius: 8px; background: #7c8cff; color: white; border: none; cursor: pointer; font-weight: 600; font-size: 13px; box-shadow: 0 4px 12px rgba(124, 140, 255, 0.4); transition: filter 0.2s;">Continue anyway</button>
        </div>
      </div>`;

    document.getElementById('tz-btn-cancel').onclick = () => {
      hud.remove();
      resolve(false);
    };
    document.getElementById('tz-btn-ok').onclick = () => {
      hud.remove();
      resolve(true);
    };

    document.getElementById('tz-btn-cancel').onmouseover = (e) => e.target.style.background = 'rgba(255,255,255,0.15)';
    document.getElementById('tz-btn-cancel').onmouseout = (e) => e.target.style.background = 'rgba(255,255,255,0.1)';
    document.getElementById('tz-btn-ok').onmouseover = (e) => e.target.style.filter = 'brightness(1.1)';
    document.getElementById('tz-btn-ok').onmouseout = (e) => e.target.style.filter = 'none';
  });
}
