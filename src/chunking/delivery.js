...
import { getSendLimit, needsSendSplit } from '../shared/brand.js';
import { getPasteLimit } from '../shared/brand.js';
...
/**
 * ≤25k chars → one message in compose box (user sends once).
 * >25k chars → multi-turn: each send part holds up to ~25k chars of body + ACK header.
 */
export async function deliverMarkdown(markdown, adapter, callbacks = {}) {
  ...
  const sendLimit = adapter.getPerMessageCharLimit?.() ?? getSendLimit(adapter.id);
  const pasteLimit = getPasteLimit(adapter.id);

  const existing = readCompose(compose).trim();
  const hasExisting = existing.length > EXISTING_MIN;
  const combined = hasExisting ? `${existing}\n\n${markdown}` : markdown;

  if (needsSendSplit(combined.length) || combined.length > sendLimit) {
