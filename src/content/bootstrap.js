...
import { showActiveBadge, hideActiveIndicators, setStatus, clearStatus, lockHud, unlockHud, askConfirm } from '../ui/glass-ui.js';
import { detectFileKind } from '../parsing/types.js';
...
async function handleFiles(files) {
  ...
  const supported = files.filter((f) => detectFileKind(f.name) !== 'unknown');
  if (!supported.length) {
    setStatus('Unsupported file', 'warn', { detail: 'Use supported documents or code files.' });
    return;
  }
  ...
  try {
    ...
    const sendLimit = settings.sendCharLimit || 25000;
    const willSplit = combined.length > sendLimit;

    if (willSplit) {
      lockHud();
      const proceed = await askConfirm(
        'Large Context Warning',
        `Sending ${combined.length.toLocaleString()} characters. This may degrade the AI's performance and cause it to lose context. We recommend sending only the parts you are currently working on.`
      );
      if (!proceed) {
        unlockHud();
        setStatus('Cancelled', 'info', { detail: 'Select fewer files and try again.' });
        return;
      }

      setStatus('Large document — splitting required', 'info', {
        detail: `${combined.length.toLocaleString()} chars · ${sendLimit.toLocaleString()} limit per send`,
        progress: 50,
        center: true,
        lock: true,
      });
